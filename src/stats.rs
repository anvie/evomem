...
pub fn stats(store: &Store) -> Result<StatsResponse> {
    ...
    let pages = one("SELECT COUNT(*) FROM pages WHERE deleted_at IS NULL")?;
    let deleted_pages = one("SELECT COUNT(*) FROM pages WHERE deleted_at IS NOT NULL")?;
    let chunks = one(
        "SELECT COUNT(*) FROM chunks c JOIN pages p ON p.id = c.page_id WHERE p.deleted_at IS NULL",
    )?;
    let indexed_words = one("SELECT COUNT(DISTINCT word) FROM word_index")?;
    let links = one("SELECT COUNT(*) FROM links")?;
    let dangling_links = one("SELECT COUNT(*) FROM links WHERE dst_page_id IS NULL")?;
    ...
    let pages_by_source = pairs(
        "SELECT source_dir, COUNT(*) FROM pages WHERE deleted_at IS NULL
